...
const COMMERCIAL_AMENITIES = [
  { label: 'Power Backup', icon: MdFlashOn },
  { label: 'Lift', icon: MdArchitecture },
  { label: 'Parking', icon: MdLocalParking },
  { label: 'Security', icon: MdSecurity },
  { label: 'Fire Safety', icon: MdFireExtinguisher },
  { label: 'Water Storage', icon: MdPool },
  { label: 'Visitor Parking', icon: MdLocalParking }
];

const COMMERCIAL_FEATURES = [
  { label: 'Central AC', icon: MdAcUnit },
  { label: 'Wi-Fi', icon: MdWifi },
  { label: 'Meeting Room', icon: MdMeetingRoom },
  { label: 'Pantry', icon: MdKitchen },
  { label: 'Reception', icon: MdStorefront },
  { label: 'Private Washroom', icon: MdWc }
];
...
export const Amenities = ({ formData, toggleArrayItem }) => {
  ...
  const renderAddButton = (category) => {
    ...
    const fieldName = category === 'buildingAmenities' ? 'building' : 'unit';
  ...
  return (
    <section id="amenities" className="scroll-mt-28 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
      ...
      <div className="space-y-8">
        ...
        <div>
          <h3 className="text-xs font-bold text-slate-400 mb-4 uppercase tracking-wider">Building Features</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            {formData.buildingAmenities
              .filter(val => !COMMERCIAL_AMENITIES.some(a => a.label === val))
              .map((customLabel) => (
                <AmenityCard 
                  key={customLabel}
                  icon={MdAdd} 
        ...
        <div>
          <h3 className="text-xs font-bold text-slate-400 mb-4 uppercase tracking-wider">Unit Features</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            {formData.inFlatFeatures
              .filter(val => !COMMERCIAL_FEATURES.some(f => f.label === val))
              .map((customLabel) => (
                <AmenityCard 
                  key={customLabel}
                  icon={MdAdd} 
