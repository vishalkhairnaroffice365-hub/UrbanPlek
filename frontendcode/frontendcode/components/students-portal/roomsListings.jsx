...
export default function StudentRoomsListings() {
  ...
  const fetchListings = useCallback(async (currentPage, isLoadMore = false) => {
    ...
    const params = new URLSearchParams({
      type: 'student',
      subtype: 'Room', // Fetch only Room/Flats
      page: currentPage,
    ...
    <div className="min-h-screen flex flex-col text-[#111318] relative overflow-hidden">
      ...
      <main className="relative z-10 flex-1 flex justify-center py-4 px-4 lg:px-8">
        <div className="flex flex-col max-w-[1100px] w-full">
          ...
          {/* Header Section */}
          <div id="flats" className="flex flex-col gap-8 mb-10 items-start scroll-mt-20">
            <div className="animate-urban-fade-left">
              <h2 className="text-[#111318] text-4xl font-extrabold tracking-tight">Premium Student Rooms</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading && listings.length === 0 ? (
              ...
              listings.map((listing) => (
                <RoomCard key={listing.db_id} listing={listing} />
              ))
            ) : (
