...
import { INITIAL_DATA, BACKEND_URL } from '../../../constants/constants';
...
const INITIAL_RESIDENTIAL_DATA = {
  ...INITIAL_DATA,
  price: '',
  images: [],
  latitude: '',
...
export default function ListPropertyFormHero() {
  const [formData, setFormData] = useState(INITIAL_RESIDENTIAL_DATA);
  ...
  const handleSubmit = () => {
    const requiredFields = [
      ...
      { key: 'price', label: 'Expected Price / Rent' },
      { key: 'location', label: 'Location' },
      { key: 'bhk', label: 'BHK Configuration' },
      { key: 'area', label: 'Super Built-up Area' },
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append('type', 'residential');
        ...
        <section className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-bold text-slate-900">
              Expected Price / Rent (₹) <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
              value={formData.price}
              onChange={(e) => handleInputChange('price', e.target.value)}
              placeholder="Enter expected price or rent"
