...
export default function ListPropertyFormHero() {
  ...
  const handleSubmit = () => {
    const requiredFields = [
      { key: 'title', label: 'Enter a property title' },
      { key: 'propertyType', label: 'Select the property type' },
      { key: 'listingType', label: 'Choose listing type (sale or rent)' },
      { key: 'location', label: 'Provide the property location' },
      { key: 'price', label: 'Enter the expected price' },
      { key: 'area', label: 'Enter the super built-up area' },
      { key: 'floor', label: 'Specify the floor number' },
      { key: 'furnishing', label: 'Select the furnishing status' },
      { key: 'images', label: 'Upload at least one property image' },
      { key: 'longitude', label: 'Pin the property on the map' },
      { key: 'fullName', label: 'Enter your full name' },
      { key: 'whatsapp', label: 'Provide your WhatsApp number' },
      { key: 'visitTiming', label: 'Select preferred visit timings' },
    ];
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append('type', 'commercial');
      ...
      if (response.ok) {
        alert('Listing submitted successfully! Our team will verify it and the page will now refresh.');
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-light)] font-[var(--font-sans)] text-[#111318]">
      ...
      <main className="max-w-[800px] mx-auto px-6 py-8 space-y-10 pb-32 animate-slide-up pt-24">
        ...
        <section>
          <h1 className="text-3xl font-black tracking-tight mb-2 text-slate-900">List Your Commercial Property</h1>
          <p className="text-slate-500 text-base font-medium">Reach businesses and investors in Nashik with a premium listing.</p>
        </section>

        <BasicDetails formData={formData} handleInputChange={handleInputChange} />
        <TechnicalSpecs formData={formData} handleInputChange={handleInputChange} />
        <Amenities formData={formData} toggleArrayItem={toggleArrayItem} />
  ...
  return (
    <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 scale-100">
        <h3 className="text-2xl font-bold mb-6 text-slate-900">Review Property Details</h3>
        ...
        <div className="space-y-6">
          ...
          {(data.buildingAmenities?.length > 0 || data.inFlatFeatures?.length > 0) && (
            <div className="space-y-3">
              <h4 className="font-bold text-slate-900 border-b border-slate-100 pb-2">Amenities & Features</h4>
              
              {data.buildingAmenities?.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Building Features</p>
                  <div className="flex flex-wrap gap-2">
                    {data.buildingAmenities.map(item => (
              ...
              {data.inFlatFeatures?.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Unit Features</p>
                  <div className="flex flex-wrap gap-2">
                    {data.inFlatFeatures.map(item => (
                      <span key={item} className="bg-blue-50 text-[var(--color-primary)] px-3 py-1 rounded-lg text-xs font-bold">{item}</span>
                    ))}
                  </div>
                </div>
