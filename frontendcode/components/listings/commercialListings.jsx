...
const CommercialCard = ({ listing }) => {
  ...
  const displayTitle = title || name || "Untitled Commercial Space";
...
export default function CommercialListings() {
  ...
  useEffect(() => {
    ...
    if (lat && lng && category === 'Commercial') {
  ...
  const fetchListings = useCallback(async (currentPage, isLoadMore = false) => {
    ...
    let endpoint = `${BACKEND_URL}/api/listings`;
    if (searchLocation && searchLocation.timeFilter) {
      endpoint = `${BACKEND_URL}/api/search`;
    }
    ...
    <div className="min-h-screen flex flex-col text-[#111318] relative overflow-hidden">
      ...
      <main className="relative z-10 flex-1 flex justify-center py-8 px-4 lg:px-8">
        <div className="flex flex-col max-w-[1100px] w-full">
          ...
          {/* Header Section */}
          <div id="commercial" className="flex flex-col gap-8 mb-10 items-start scroll-mt-20">
            <div className="animate-urban-fade-left">
              <h2 className="text-[#111318] text-4xl font-extrabold tracking-tight">Top Commercial Spaces</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading && listings.length === 0 ? (
              ...
              listings.map((listing) => <CommercialCard key={listing.db_id} listing={listing} />)
            ) : (
              <div className="col-span-full text-center py-10">
                <p className="text-gray-500 font-medium">No commercial spaces found matching your criteria.</p>
